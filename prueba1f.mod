...
% Parameters:
parameters alpha, rho_a, beta, delta, sigma_a, mu, rho_tau, rho_g, sigma_tau, theta_g, sigma_g, etha, zeta, psi, tau_ee, g_ee, k_ee, w_ee, rk_ee, varphi;
...
%5
exp(w)=((psi*(exp(l)^zeta))/((exp(g)^etha)*(1-tau)))^(1-varphi)*exp(w(-1))^varphi;
%6
